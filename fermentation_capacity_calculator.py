...
def _deterministic_per_strain_capacity(strain, reactors_assigned, lines_assigned, cfg):
    ...
    feasible_batches = (
        ...
        if (reactors_assigned > 0 and lines_assigned > 0)
        else 0.0
    )
    ...
    if reactors_assigned > 0:
        up_util = min(
            1.0, _safe_div(feasible_batches * ct_up, reactors_assigned * up_hours)
        )
    if lines_assigned > 0:
        ds_util = min(
            1.0, _safe_div(feasible_batches * ct_ds, lines_assigned * ds_hours)
        )
...
def calculate_deterministic_capacity(
    ...
    cfg: EquipmentConfig,
    reactor_allocation_policy: Literal[
        "equal", "proportional", "inverse_ct"
    ] = "inverse_ct",
    ...
    if cfg.reactors_per_strain is not None:
        reactors_alloc = {
            name: float(cfg.reactors_per_strain.get(name, 0)) for name in names
        }
    else:
        if cfg.reactors_total is None:
            ...
            ct_up = np.array(
                [s.fermentation_time_h + s.turnaround_time_h for s in strains],
                dtype=float,
            )
            ...
            if total_R >= len(names):
                ...
                int_alloc = _allocation_from_totals(
                    names,
                    int(round(total_R)),
                    weights,
                ...
                reactors_alloc = _share_allocation(
                    names,
                    total_R,
                    weights,
                    "equal" if weights is None else "proportional",
    ...
    totals = {
        ...
        "weighted_up_utilization": float(
            np.average(
                df["up_utilization"], weights=np.maximum(df["reactors_assigned"], 1e-6)
            )
        ),
        "weighted_ds_utilization": float(
            np.average(
                df["ds_utilization"], weights=np.maximum(df["ds_lines_assigned"], 1e-6)
            )
        ),
    ...
    if "annual_kg_good" in df.columns:
        totals["total_annual_kg_good"] = float(
            df.get("annual_kg_good", pd.Series(dtype=float)).sum()
        )
    return df, totals
...
def monte_carlo_capacity(
    ...
    for _ in range(n_sims):
        ...
        for s in strains:
            ferm = float(
                _sample_lognormal_from_mean_cv(
                    s.fermentation_time_h, s.cv_ferm or 0.0, 1
                )[0]
            )
            turn = float(
                _sample_lognormal_from_mean_cv(
                    s.turnaround_time_h, s.cv_turn or 0.0, 1
                )[0]
            )
            down = float(
                _sample_lognormal_from_mean_cv(
                    s.downstream_time_h, s.cv_down or 0.0, 1
                )[0]
            )
            s_tmp = StrainSpec(
                name=s.name,
        ...
        results.append(
            {
                "feasible_batches": plant_batches,
                "good_batches": plant_good,
                "annual_kg_good": plant_kg,
            }
        )
