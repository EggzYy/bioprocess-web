...
class AppState {
  ...
  saveToLocalStorage() {
    localStorage.setItem(
      "bioprocess_scenario",
      JSON.stringify(this.currentScenario),
    );
  }
...
function collectFormData() {
  ...
  scenario.name =
    document.getElementById("scenarioName")?.value || "Test Scenario";
  scenario.description =
    document.getElementById("scenarioDescription")?.value || "";
  scenario.target_tpa =
    parseFloat(document.getElementById("targetTPA")?.value) || 10;
  ...
  strainRows.forEach((row) => {
    ...
    const strain = {
      ...
      fermentation_time_h:
        parseFloat(
          row.querySelector(`[name="ferm_time_${strainId}"]`)?.value,
        ) || 18,
      turnaround_time_h:
        parseFloat(
          row.querySelector(`[name="turn_time_${strainId}"]`)?.value,
        ) || 9,
      downstream_time_h:
        parseFloat(
          row.querySelector(`[name="down_time_${strainId}"]`)?.value,
        ) || 4,
      yield_g_per_L:
        parseFloat(row.querySelector(`[name="yield_${strainId}"]`)?.value) ||
        10,
      media_cost_usd:
        parseFloat(
          row.querySelector(`[name="media_cost_${strainId}"]`)?.value,
        ) || 100,
      cryo_cost_usd:
        parseFloat(
          row.querySelector(`[name="cryo_cost_${strainId}"]`)?.value,
        ) || 50,
      utility_rate_ferm_kw:
        parseFloat(
          row.querySelector(`[name="util_ferm_${strainId}"]`)?.value,
        ) || 300,
      utility_rate_cent_kw:
        parseFloat(
          row.querySelector(`[name="util_cent_${strainId}"]`)?.value,
        ) || 15,
      utility_rate_lyo_kw:
        parseFloat(row.querySelector(`[name="util_lyo_${strainId}"]`)?.value) ||
        1.5,
      utility_cost_steam: 0.0228,
      cv_ferm:
        parseFloat(row.querySelector(`[name="cv_ferm_${strainId}"]`)?.value) ||
        0.1,
      cv_turn:
        parseFloat(row.querySelector(`[name="cv_turn_${strainId}"]`)?.value) ||
        0.1,
      cv_down:
        parseFloat(row.querySelector(`[name="cv_down_${strainId}"]`)?.value) ||
        0.1,
    };

    // Add optional fields if present
    const respirationType = row.querySelector(
      `[name="respiration_type_${strainId}"]`,
    )?.value;
    if (respirationType) strain.respiration_type = respirationType;

    const requiresTff = row.querySelector(
      `[name="requires_tff_${strainId}"]`,
    )?.value;
    if (requiresTff) strain.requires_tff = requiresTff === "true";

    const dsComplexity = row.querySelector(
      `[name="downstream_complexity_${strainId}"]`,
    )?.value;
  ...
  console.log(
    "Volume checkboxes found:",
    document.querySelectorAll(".volume-option").length,
  );
  console.log(
    "Volume checkboxes checked:",
    document.querySelectorAll(".volume-option:checked").length,
  );
  console.log("Volume options collected:", volumeOptions);

  // Always ensure we have at least one volume option
  const baseFermenterVol =
    parseFloat(document.getElementById("baseFermenterVolume")?.value) || 2000;

  // If no checkboxes are selected, use the base fermenter volume
  if (volumeOptions.length === 0) {
    volumeOptions.push(baseFermenterVol);
    console.log(
      "No volume checkboxes selected, using base fermenter volume:",
      baseFermenterVol,
    );
  }
  ...
  scenario.volumes = {
    ...
    working_volume_fraction:
      parseFloat(document.getElementById("workingVolumeFraction")?.value) ||
      0.8,
    seed_fermenter_ratio:
      parseFloat(document.getElementById("seedFermenterRatio")?.value) || 0.125,
    media_tank_ratio:
      parseFloat(document.getElementById("mediaTankRatio")?.value) || 1.25,
  };
  ...
  scenario.equipment = {
    ...
    reactor_allocation_policy:
      document.getElementById("reactorAllocationPolicy")?.value || "inverse_ct",
    ds_allocation_policy:
      document.getElementById("dsAllocationPolicy")?.value || "inverse_ct",
    shared_downstream:
      document.getElementById("sharedDownstream")?.checked || true,
  };
  ...
  scenario.economics = {
    discount_rate: parseFloat(discountRateEl?.value) / 100 || 0.1,
    tax_rate: parseFloat(taxRateEl?.value) / 100 || 0.25,
    depreciation_years:
      parseInt(document.getElementById("depreciationYears")?.value) || 10,
    project_lifetime_years:
      parseInt(document.getElementById("projectLifetimeYears")?.value) || 15,
    variable_opex_share:
      parseFloat(document.getElementById("variableOpexShare")?.value) / 100 ||
      0.85,
    maintenance_pct_of_equip:
      parseFloat(document.getElementById("maintenancePctOfEquip")?.value) /
        100 || 0.09,
    ga_other_scale_factor:
      parseFloat(document.getElementById("gaScaleFactor")?.value) || 10.84,
  };
  ...
  scenario.labor = {
    plant_manager_salary:
      parseFloat(document.getElementById("plantManagerSalary")?.value) ||
      104000,
    fermentation_specialist_salary:
      parseFloat(
        document.getElementById("fermentationSpecialistSalary")?.value,
      ) || 39000,
    downstream_process_operator_salary:
      parseFloat(document.getElementById("dsProcessOperatorSalary")?.value) ||
      52000,
    general_technician_salary:
      parseFloat(document.getElementById("generalTechnicianSalary")?.value) ||
      32500,
    qaqc_lab_tech_salary:
      parseFloat(document.getElementById("qaqcLabTechSalary")?.value) || 39000,
    maintenance_tech_salary:
      parseFloat(document.getElementById("maintenanceTechSalary")?.value) ||
      39000,
    utility_operator_salary:
      parseFloat(document.getElementById("utilityOperatorSalary")?.value) ||
      39000,
    logistics_clerk_salary:
      parseFloat(document.getElementById("logisticsClerkSalary")?.value) ||
      39000,
    office_clerk_salary:
      parseFloat(document.getElementById("officeClerkSalary")?.value) || 32500,
  ...
  scenario.opex = {
    electricity_usd_per_kwh:
      parseFloat(document.getElementById("electricityUsdPerKwh")?.value) ||
      0.107,
    steam_usd_per_kg:
      parseFloat(document.getElementById("steamUsdPerKg")?.value) || 0.0228,
    water_usd_per_m3:
      parseFloat(document.getElementById("waterUsdPerM3")?.value) || 0.002,
    natural_gas_usd_per_mmbtu:
      parseFloat(document.getElementById("naturalGasUsdPerMmbtu")?.value) ||
      3.5,
    raw_materials_markup:
      parseFloat(document.getElementById("rawMaterialsMarkup")?.value) || 1.0,
    utilities_efficiency:
      parseFloat(document.getElementById("utilitiesEfficiency")?.value) || 0.85,
  };

  // Collect CAPEX parameters
  scenario.capex = {
    land_cost_per_m2:
      parseFloat(document.getElementById("landCostPerM2")?.value) || 500,
    building_cost_per_m2:
      parseFloat(document.getElementById("buildingCostPerM2")?.value) || 2000,
    fermenter_base_cost:
      parseFloat(document.getElementById("fermenterBaseCost")?.value) || 150000,
    fermenter_scale_exponent:
      parseFloat(document.getElementById("fermenterScaleExponent")?.value) ||
      0.6,
    centrifuge_cost:
      parseFloat(document.getElementById("centrifugeCost")?.value) || 200000,
    tff_skid_cost:
      parseFloat(document.getElementById("tffSkidCost")?.value) || 150000,
    lyophilizer_cost_per_m2:
      parseFloat(document.getElementById("lyophilizerCostPerM2")?.value) ||
      50000,
    utilities_cost_factor:
      parseFloat(document.getElementById("utilitiesCostFactor")?.value) / 100 ||
      0.25,
    installation_factor:
      parseFloat(document.getElementById("installationFactor")?.value) / 100 ||
      0.15,
    contingency_factor:
      parseFloat(document.getElementById("contingencyFactor")?.value) / 100 ||
      0.125,
    working_capital_months:
      parseInt(document.getElementById("workingCapitalMonths")?.value) || 3,
  };
  ...
  scenario.prices = {
    product_prices: {
      ...
      lacto_bifido:
        parseFloat(document.getElementById("priceLactoBifido")?.value) || 400,
      bacillus:
        parseFloat(document.getElementById("priceBacillus")?.value) || 400,
  ...
  // Enable equipment optimization and multiobjective optimization by default
  // This ensures volume options from checkboxes are properly used in grid search
  scenario.optimize_equipment = true;
  scenario.use_multiobjective = true;

  // Collect optimization parameters
  scenario.optimization = {
    enabled: document.getElementById("optimizationEnabled")?.checked || true, // Default to true
    simulation_type:
      document.getElementById("simulationType")?.value || "deterministic",
    objectives: [],
    min_tpa: parseFloat(document.getElementById("minTpa")?.value) || null,
    max_capex_usd:
      parseFloat(document.getElementById("maxCapex")?.value) * 1000000 || null,
    min_utilization:
      parseFloat(document.getElementById("minUtilization")?.value) / 100 ||
      null,
    max_payback:
      parseFloat(document.getElementById("maxPayback")?.value) || null,
    max_evaluations:
      parseInt(document.getElementById("maxEvaluations")?.value) || 100,
    population_size:
      parseInt(document.getElementById("populationSize")?.value) || 50,
    n_generations:
      parseInt(document.getElementById("nGenerations")?.value) || 100,
    n_monte_carlo_samples:
      parseInt(document.getElementById("nMonteCarloSamples")?.value) || 1000,
    confidence_level:
  ...
  document
    .querySelectorAll('#optimization-tab input[type="checkbox"]:checked')
    .forEach((checkbox) => {
      if (
        checkbox.value &&
        ["npv", "irr", "capex", "opex", "payback"].includes(checkbox.value)
      ) {
  ...
  scenario.sensitivity = {
    ...
    parameters: [],
    delta_percentage:
      parseFloat(document.getElementById("deltaPercentage")?.value) / 100 ||
      0.1,
    grid_points: parseInt(document.getElementById("gridPoints")?.value) || 5,
    n_samples:
      parseInt(document.getElementById("mcSensitivitySamples")?.value) || 1000,
  };

  // Collect sensitivity parameters to analyze
  document
    .querySelectorAll('#sensitivity-tab input[type="checkbox"]:checked')
    .forEach((checkbox) => {
      if (checkbox.value) {
  ...
  scenario.assumptions = {
    hours_per_year:
      parseFloat(document.getElementById("hoursPerYear")?.value) || 8760,
    upstream_availability:
      parseFloat(document.getElementById("upstreamAvailability")?.value) /
        100 || 0.92,
    downstream_availability:
      parseFloat(document.getElementById("downstreamAvailability")?.value) /
        100 || 0.9,
    quality_yield:
      parseFloat(document.getElementById("qualityYield")?.value) / 100 || 0.98,
...
function addSelectedStrain() {
  ...
  showAlert(
    `${strainName} added from database with pre-configured values.`,
    "success",
  );
}
...
function removeStrain(strainId) {
  ...
  if (element) {
    element.remove();
    appState.activeStrains = appState.activeStrains.filter(
      (id) => id !== strainId,
    );
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (defaults.available_strains) {
        ...
        appState.availableStrains = Object.entries(
          defaults.available_strains,
        ).map(([name, data]) => ({
          name: name,
          data: data,
...
async function runScenario() {
  ...
  try {
    ...
    showAlert(
      "Failed to run scenario. Please check your connection.",
      "danger",
    );
  } finally {
    hideLoading();
...
async function runOptimization() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const error = await response.json();
      showAlert(
        `Error: ${error.detail || "Failed to run optimization"}`,
        "danger",
      );
    }
  } catch (error) {
    ...
    showAlert(
      "Failed to run optimization. Please check your connection.",
      "danger",
    );
  } finally {
    hideLoading();
...
async function runSensitivityAnalysis() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const error = await response.json();
      showAlert(
        `Error: ${error.detail || "Failed to run sensitivity analysis"}`,
        "danger",
      );
    }
  } catch (error) {
    ...
    showAlert(
      "Failed to run sensitivity analysis. Please check your connection.",
      "danger",
    );
  } finally {
    hideLoading();
...
function collectOptimizationParams() {
  ...
  return {
    objective:
      document.querySelector('[name="optimization_objective"]')?.value || "npv",
    constraints: [],
    variables: [],
  };
}
...
function collectSensitivityParams() {
  ...
  const checkboxes = document.querySelectorAll(
    '#sensitivityParams input[type="checkbox"]:checked',
  );
...
function createResultCharts(results) {
  try {
    ...
    showAlert(
      "Charts could not be displayed. Check console for details.",
      "warning",
    );
  }
}
...
function saveScenario() {
  ...
  link.setAttribute(
    "download",
    `scenario_${scenario.name || "unnamed"}_${Date.now()}.json`,
  );
...
function loadPreset(presetName) {
  ...
  if (preset) {
    ...
    if (container) {
      container.innerHTML =
        '<p class="text-muted">No strains added yet. Use the controls above to add strains.</p>';
      appState.activeStrains = [];
    }

    showAlert(`Loaded preset: ${preset.name}`, "info");
    showAlert(
      "Add the specified strains from the strain selector to complete the setup",
      "info",
    );
  }
}
  ...
  try {
    ...
    if (format === "json") {
      ...
      if (response.ok) {
        ...
        window.URL.revokeObjectURL(url);
        showAlert(
          `${format.toUpperCase()} results exported successfully!`,
          "success",
        );
      } else {
        const errorText = await response.text();
